...
then 
	...
	echo -e "\033[31m[FAIL]\033[0m\t\t I cannot find a container with the name lab2. Did you use \033[1mdocker run lab2\033[0m with the appropriate options to run it?" && exit 4
fi
...
then
	...
	echo -e "\033[31m[FAIL]\033[om\t\t Nothing listening on localhost port 8081. You should probably delete the container and run it again, using \033[1mdocker stop lab2; docker rm lab2; docker run -p 8081:80 --name lab2 lab2" && exit 2
fi
